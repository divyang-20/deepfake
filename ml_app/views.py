...
def plot_heat_map(i, model, img, path="./", video_file_name=""):
    ...
    out = np.dot(
        fmap[i].detach().cpu().numpy().reshape((nc, h * w)).T, weight_softmax[idx, :].T
    )
        ...
        if video_upload_form.is_valid():
            ...
            if video_content_type in settings.CONTENT_TYPES:
                if video_file.size > int(settings.MAX_UPLOAD_SIZE):
                    ...
                    return render(
                        request, index_template_name, {"form": video_upload_form}
                    )
...
def predict_page(request):
    if request.method == "GET":
        ...
        if settings.DEBUG == False:
            ...
            production_video_name = "/".join(
                [str(elem) for elem in production_video_name]
            )
